...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  // YOUR CODE HERE
  // loop over state space for all positions x (convolution):
  for (float j = 0; j < map_size; ++j) {
    ...
    // -> the probability of moving from x_t-1 to x_t
    float transition_prob =
        Helpers::normpdf(distance_ij, movement, control_stdev);
...
vector<float> initialize_priors(int map_size, vector<float> landmark_positions,
  ...
  for (int i = 0; i < landmark_positions.size(); ++i) {
    for (float j = 1; j <= position_stdev; ++j) {
      priors.at(int(j + landmark_positions[i] + map_size) % map_size) +=
          1.0 / norm_term;
      priors.at(int(-j + landmark_positions[i] + map_size) % map_size) +=
          1.0 / norm_term;
    }
