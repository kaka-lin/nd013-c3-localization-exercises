...
vector<float> initialize_priors(int map_size, vector<float> landmark_positions,
  ...
  float normalize_value =
      1.0 / (landmark_positions.size() * (position_stdev * 2 + 1));

  for (int i = 0; i < landmark_positions.size(); ++i) {
    priors[landmark_positions[i]] = normalize_value;
    for (float j = 1; j <= position_stdev; ++j) {
      priors[int(landmark_positions[i] + j + map_size) % map_size] = normalize_value;
      priors[int(landmark_positions[i] - j + map_size) % map_size] = normalize_value;
    }
  }
